...
async function createClient({ id = null, full_name, email, phone, created_by_channel, metadata = {} }) {
  if (id) {
    const res = await pool.query(
      `INSERT INTO app.clients (id, full_name, email, phone, created_by_channel, metadata) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [id, full_name, email, phone || null, created_by_channel || null, metadata]
    );
    return mapClientRow(res.rows[0]);
  }
  const res = await pool.query(
    `INSERT INTO app.clients (full_name, email, phone, created_by_channel, metadata) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
    [full_name, email, phone || null, created_by_channel || null, metadata]
  );
  return mapClientRow(res.rows[0]);
}
...
async function updateClient(id, { full_name, email, phone, created_by_channel, is_active, metadata }) {
  const res = await pool.query(
    `UPDATE app.clients SET full_name = COALESCE($1, full_name), email = COALESCE($2, email), phone = COALESCE($3, phone), created_by_channel = COALESCE($4, created_by_channel), is_active = COALESCE($5, is_active), metadata = COALESCE($6, metadata), updated_at = now() WHERE id = $7 RETURNING *`,
    [full_name, email, phone, created_by_channel, is_active, metadata, id]
  );
  return mapClientRow(res.rows[0]);
}
