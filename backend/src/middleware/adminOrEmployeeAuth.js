...
const EMPLOYEE_ROLES = new Set(['admin', 'employe', 'employé', 'employee', 'staff']);

// Middleware : session admin OU employé valide, non expirée, compte actif et non supprimé
module.exports = async function adminOrEmployeeAuth(req, res, next) {
  try {
    ...
    const ok = roles.some(r => EMPLOYEE_ROLES.has(r));
